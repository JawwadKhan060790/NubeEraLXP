...
/// <summary>
/// Verifies connectivity to the existing database (nubeera_db).
...
public sealed class DatabaseManager
{
    ...
    public async Task EnsureDatabaseAsync()
    {
        ...
        foreach (var table in required)
        {
            ...
            if (!exists)
                throw new Exception($"Required table '{table}' not found in nubeera_db. " +
                                    "Run the application migrations first.");
        }

        Console.WriteLine("         Database nubeera_db: connected OK");
