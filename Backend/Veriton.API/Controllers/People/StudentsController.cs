...
public class StudentsController : ControllerBase
{
    ...
    [Authorize(Policy = "TeacherOnly")]
    public async Task<IActionResult> ResetParentPassword(Guid id, [FromBody] Veriton.API.Controllers.Auth.ResetPasswordBody? request)
    {
    ...
    [Authorize(Policy = "TeacherOnly")]
    public async Task<IActionResult> ResetStudentPassword(Guid id, [FromBody] Veriton.API.Controllers.Auth.ResetPasswordBody? request)
    {
        if (string.IsNullOrWhiteSpace(request?.Password))
            return BadRequest(new { message = "Password is required" });
