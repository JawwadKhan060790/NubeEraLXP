...
public class TeacherLearningPathService : ITeacherLearningPathService
{
    ...
    public async Task<TeacherGradeStudentListDto> GetGradeStudentListAsync(Guid teacherId, Guid gradeId, Guid? sectionId = null)
    {
        ...
        var schedulers = await _schedulerRepo.GetAllAsync(q =>
            q.Where(s => s.TeacherId == teacherId && s.GradeId == gradeId && (!sectionId.HasValue || s.SectionId == sectionId.Value)));
        ...
        var periods = await _periodRepo.GetAllAsync(q =>
            q.Where(p => p.GradeId == gradeId && p.TeacherId == teacherId && (!sectionId.HasValue || schedulerIds.Contains(p.SchedulerId))));
        int totalPeriodsConducted = periods.Count(p => p.Status == PeriodStatus.Completed);

        var sectionsMap = (await _sectionRepo.GetAllAsync(q => q.Where(sec => sec.GradeId == gradeId && sec.IsActive)))
