...
import { Award, BookOpen, Calendar, Check, ChevronDown, ChevronRight, Clock, Edit, Plus, Search, Trash, Trash2, X } from 'lucide-react';
                ...
                pagedExams.map((exam) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSelected && <ChevronRight className="w-3.5 h-3.5 text-indigo-500 flex-shrink-0" />}
                    </div>
                  );
                })
              )}
            ...
            {selectedExam ? (
              <div className="space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                  <div className={`h-1.5 w-full ${new Date(selectedExam.date) >= new Date() ? 'bg-gradient-to-r from-emerald-400 to-teal-500' : 'bg-gradient-to-r from-slate-300 to-slate-400'}`} />
                  <div className="p-5 space-y-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                    <div className="flex items-center gap-3.5">
                      <div className="w-16 h-16 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center text-indigo-700 text-sm font-black uppercase flex-shrink-0">
                        MCQ
                      </div>
                      <div>
                        <h2 className="text-base font-black text-slate-800 uppercase tracking-tight leading-tight">{selectedExam.title}</h2>
                      ...
                      {user?.utype === 'student' ? (
                        ...
                        <>
                          <button
                            ...
                            className="px-3 py-2 border border-slate-200 hover:border-indigo-200 hover:bg-indigo-50 rounded-lg text-slate-600 hover:text-indigo-700 text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 transition-all"
                          ...
                          <button
                            ...
                            className="p-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg text-indigo-700 transition-all flex items-center justify-center"
                            title="Edit Exam"
                          >
                          ...
                          <button
                            ...
                            className="p-2 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg text-rose-600 transition-all flex items-center justify-center"
                            title="Delete Exam"
                          >
