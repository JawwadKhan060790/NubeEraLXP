...
import { AlertCircle, Calendar, Check, ChevronDown, ChevronRight, Droplets, Edit, Eye, EyeOff, GraduationCap, Key, Lock, Mail, MapPin, Phone, Plus, School, Search, Trash2, User, X } from 'lucide-react';
...
const Students: React.FC = () => {
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/users/${resettingStudent.user_id}/reset-password`, {
        password: resetPasswordData.password
      });
      toast.success(`Password updated for ${resettingStudent.first_name} ${resettingStudent.last_name}`);
        ...
        <div className="flex flex-wrap gap-2.5">
          ...
          {isAdmin && (
            <button
              ...
              className="text-white px-4 py-2.5 rounded-[4px] font-bold uppercase tracking-widest text-[10px] transition-all outline-none flex items-center justify-center gap-1.5 shadow-md hover:shadow-purple-400/40 hover:brightness-110 hover:scale-[1.02] active:scale-[0.98]"
            >
              &#8593; Academic Promotion
            </button>
          )}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-slate-500 dark:text-[#94a3b8] font-medium truncate mt-0.5">
                          {student.grade_display || (student.section_code ? `${student.grade_name} - ${student.section_code}` : student.grade_name) || 'Unassigned'} · {student.student_id}
                        </div>
                      </div>
            ...
            {selectedStudent ? (
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-[#1e293b] border border-slate-200 dark:border-[#334155] rounded-2xl shadow-sm dark:shadow-[0_4px_20px_rgba(0,0,0,0.3)] overflow-hidden animate-in fade-in slide-in-from-right-2 duration-300">
                  {/* Color band */}
                  <div className={`h-1.5 w-full ${selectedStudent.is_active
                  ...
                  <div className="p-5 space-y-5">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                      <div className="flex items-center gap-3.5">
                        ...
                        <div>
                          <h2 className="text-base font-black text-slate-800 dark:text-white tracking-tight leading-tight">{selectedStudent.full_name}</h2>
                          <p className="text-xs text-slate-500 dark:text-[#94a3b8] font-medium mt-0.5">{selectedStudent.grade_name || 'No Grade Assigned'} · {selectedStudent.school_name}</p>
            ...
            <div className="p-6 md:p-8 space-y-6 overflow-y-auto custom-scrollbar flex-1">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={e => setFormData({ ...formData, school_id: e.target.value, grade_id: '', section_id: '' })}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-500 dark:text-[#94a3b8] uppercase tracking-wider mb-2 ml-1">Student ID (AAPAR ID) <span className="text-gray-400 font-normal normal-case">(optional)</span></label>
                    <input
                      type="text"
                      ...
                      disabled={!!editingId}
                      className="w-full px-4 py-3 bg-white dark:bg-[#1e293b] border border-gray-200 dark:border-[#334155] rounded-[4px] focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all outline-none font-bold text-primary text-sm shadow-sm disabled:bg-gray-50 dark:disabled:bg-[#283548]"
                    />
